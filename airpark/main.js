...
},{"_process":194}],190:[function(require,module,exports){
...
},{"jquery":193}],192:[function(require,module,exports){
var jQuery = require('jquery');
require('./core');
...
},{"jquery":193}],196:[function(require,module,exports){
'use strict';
...
var Direction = function Direction() {
  ...
  var _AIRPORT = {
    lat: 55.61802360000001,
    lng: 12.650762799999939
  };
  ...
  function _cacheDom() {
    _domElements.address = document.getElementById('AutoComplete');
    _domElements.travelers = document.getElementById('calculator-travelers');
    _domElements.$date = (0, _jquery2.default)('#date');
    _domElements.calculate = document.getElementById('calculator-calculate');
  }

  function _events() {
    _domElements.calculate.addEventListener('click', calculateDistance);
  }

  function _initAutocomplete() {
    _autoComplete = new google.maps.places.Autocomplete(_domElements.address, {
  ...
  function calculateDistance() {
    var car = distanceMatrix('DRIVING');
    if (car == null) return;

    var train = distanceMatrix('TRANSIT');
    var bike = distanceMatrix('BIKE');

    var promise = new Promise(function (resolve, reject) {
      Promise.all([car, train, bike]).then(function (res) {
        resolve(res);

        // get all the input values
        var data = {
          kinetics: res,
          travelers: +_domElements.travelers.querySelector('input').value || 1,
          date: _domElements.$date.val() || new Date()
        };
        // Notify subscribers
        _pubsub2.default.emit('calculationDone', data);
      });
    });
    return promise;
  }

  function distanceMatrix(transportMode) {
    var address = _getEnteredAddress();

    if (address === 'error') {

      (0, _jquery2.default)(_domElements.address).focus();

      setTimeout(function () {
        (0, _jquery2.default)('#calculator').removeClass('spinner');
        (0, _jquery2.default)(_domElements.address).addClass('inputErrorClass');
        _toastr2.default.error('Vi kunne ikke finde den indtastede adresse, prøv igen');
      }, 1500);
    ...
    var promise = new Promise(function (resolve, reject) {
      service.getDistanceMatrix({
        origins: [new google.maps.LatLng(address.lat, address.lng)],
        destinations: [new google.maps.LatLng(_AIRPORT.lat, _AIRPORT.lng)],
  ...
  // Try to get the users current position
  ...
  function _addressSelect() {
    _domElements.$date.datepicker();
  }

  //calculateDistance returns a promise
  return {
    init: init,
...
},{"./lib/pubsub.js":202,"babel-polyfill":1,"jquery":193,"jquery-ui/datepicker":192,"toastr":195}],197:[function(require,module,exports){
'use strict';
...
var results = function () {
  ...
  function calculateStats(data) {
    ...
    var carPrice = +(car.distance / 1000) * _config2.default.prices.carKm + addBridge(originAddress);
  ...
  function addBridge(address) {
    var zipCode = +address.match(/\s(\d{4})\s/g)[0];
  ...
  function appendResults() {
    ...
    (0, _jquery2.default)($trainJoy).html(TRAIN_JOY.toFixed(0));
    (0, _jquery2.default)($taxiJoy).html(TAXI_JOY.toFixed(0));
    (0, _jquery2.default)($bikeJoy).html(BIKE_JOY.toFixed(0));
...
},{"./lib/config.js":201,"./lib/pubsub.js":202,"jquery":193}],198:[function(require,module,exports){
'use strict';
...
function canvas() {
  ...
  var direction = { down: false, up: false };
  ...
  function preload() {
    ...
    game.load.image('airport', 'assets/images/game/airport.png');
  ...
  function create() {
    ...
    scale = (0, _jquery2.default)(window).width() / background.width;
    game.world.setBounds(0, 0, _const.BG_WIDTH * scale, _const.BG_HEIGHT * scale - 500 * scale);

    // Add the sprites

    result = game.add.sprite(_const.START_POS.result.x * scale, _const.START_POS.result.y * scale, 'result');
    ...
    result.events.onInputDown.add(listener, this);

    function listener() {
      _gameState2.default.showResults();
    }

    // Add the clouds
    clouds = [];
    for (var i = 0; i < 50 * (0, _random.getRandom)(0.2, 1); i++) {
      clouds.push(game.add.sprite(_const.BG_WIDTH * scale * Math.random(), _const.BG_HEIGHT * scale * Math.random(), 'cloud'));
      clouds[i].scale.setTo((0, _random.getRandom)(0.2, 0.9) * scale);
      game.physics.arcade.enable(clouds[i], Phaser.Physics.ARCADE);
    }
  ...
  function update() {
    ...
    for (var i = 0; i < clouds.length; i++) {
      clouds[i].body.velocity.x = 20 * scale;
      game.world.wrap(clouds[i], 0, true);
    }

    // Animate boats
    boat1.body.velocity.x = -50 * scale;
    ...
    var bikeWithinFirst = bike.position.y >= _const.BIKE_TURN.first.y * scale && bike.position.y <= _const.BIKE_TURN.first.yTo * scale;
    var bikeWithinSecond = bike.position.y >= _const.BIKE_TURN.second.y * scale && bike.position.y <= _const.BIKE_TURN.second.yTo * scale;
    var bikeWithinThird = bike.position.y >= _const.BIKE_TURN.third.y * scale && bike.position.y <= _const.BIKE_TURN.third.yTo * scale;
    ...
    if (scrolling.down) {
      ...
      if (bikeWithinFirst) {
        ...
        bike.body.velocity.y = SPEED.bike * scale;
        bike.rotation = 0;
      }
    ...
    var taxiWithinFirst = taxi.position.y >= _const.TAXI_TURN.first.y * scale && taxi.position.y <= _const.TAXI_TURN.first.yTo * scale;
    var taxiWithinSecond = taxi.position.y >= _const.TAXI_TURN.second.y * scale && taxi.position.y <= _const.TAXI_TURN.second.yTo * scale;
    var taxiWithinThird = taxi.position.y >= _const.TAXI_TURN.third.y * scale && taxi.position.y <= _const.TAXI_TURN.third.yTo * scale;
    ...
    /* AT THE END HIDE THE VEHICLES */

    if (car.position.y > _const.START_POS.airport.y * scale + 200 * scale) {
      car.alpha = 0;
    }
    if (taxi.position.y > _const.START_POS.airport.y * scale + 200 * scale) {
      taxi.alpha = 0;
    }
...
},{"./const":199,"./gameState.js":200,"./lib/pubsub.js":202,"./random":205,"jquery":193}],199:[function(require,module,exports){
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
var CAR_TURN = exports.CAR_TURN = {
  first: { y: 1360, xTo: 1474, yTo: 1779 },
  second: { y: 4464, xTo: 751, yTo: 4891 },
  third: { y: 7437, xTo: 1756, yTo: 7815 },
  fourth: { y: 7692, xTo: 1380, yTo: 7692 }
};

var BIKE_TURN = exports.BIKE_TURN = {
...
var TAXI_TURN = exports.TAXI_TURN = {
  first: { y: 1368, xTo: 1474, yTo: 1779 },
  second: { y: 4464, xTo: 751, yTo: 4891 },
  third: { y: 7437, xTo: 1756, yTo: 7815 },
  fourth: { y: 7692, xTo: 1380, yTo: 7692 }
...
var TRAIN_TURN = exports.TRAIN_TURN = {
  first: { y: 1368, xTo: 1474, yTo: 1779 },
  second: { y: 4464, xTo: 751, yTo: 4891 },
  third: { y: 7437, xTo: 1756, yTo: 7815 },
  fourth: { y: 7692, xTo: 1380, yTo: 7692 }
...
var START_POS = exports.START_POS = {
  car: { x: 930, y: 400 },
  taxi: { x: 1000, y: 100 },
  train: { x: 800, y: 100 },
  bike: { x: 1200, y: 100 },
  result: { x: 1330, y: 11000 },
  bridge: { x: 440, y: 5300 },
...
var BG_WIDTH = exports.BG_WIDTH = 2560;
...
exports.default = gameState = {
	...
	playGame: function playGame() {
		document.addEventListener('mousewheel', this.preventDefaultFunc);
	...
	showResults: function showResults() {
		...
		_pubsub2.default.emit('showResults', []);
	},
	preventDefaultFunc: function preventDefaultFunc(e) {
		e.preventDefault();
	}
};

},{"./canvas.js":198,"./lib/pubsub.js":202,"./main.js":204,"jquery":193}],201:[function(require,module,exports){
'use strict';
...
exports.default = {
  assets: {
    ...
    sprites: './assets/images/sprites.json'
  },
...
},{"./pubsub.js":202,"jquery":193}],204:[function(require,module,exports){
'use strict';
...
var _DistanceMatrix = require('./DistanceMatrix.js');
...
require('./lib/scrollDirection.js');

require('./calculateResults.js');
...
window.onload = function (event) {
	...
	distance = new _DistanceMatrix2.default();

	_gameState2.default.top();

	(0, _canvas.canvas)();

	// When clicking scroll down on the hero screen
	// go to the calculation
	(0, _jquery2.default)('.scroll-logo').on('click', function (event) {
		var bound = hero.getBoundingClientRect();
		console.log(bound);
		(0, _jquery2.default)('body').animate({ scrollTop: bound.height }, 500);
	});

	// When clicking, calculate
	// the spinner should start
	...
	_pubsub2.default.on('calculationDone', function (res) {
		var car = res.kinetics[0];
		var transit = res.kinetics[1];
		console.log(res);

		// wait 1 second before starting the game
		// to give the user a feeling of some
		// super secret calculation taking place
		setTimeout(function () {
			(0, _jquery2.default)('#calculator').removeClass('spinner');
			_gameState2.default.playGame();
		}, 1000);
	});
};
...
},{"./DistanceMatrix.js":196,"./calculateResults.js":197,"./canvas.js":198,"./gameState.js":200,"./lib/config.js":201,"./lib/pubsub.js":202,"./lib/scrollDirection.js":203,"is-mobile":190,"jquery":193}],205:[function(require,module,exports){
"use strict";
